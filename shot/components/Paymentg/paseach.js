...
export default class Paseach extends Component {
  constructor(props) {
    super(props);
  ...
  handseach(){
    ...
    this.props.actions.getckls(userName,token,1,serv,payUser,receUser,expiTime,guar,stat,blacklist);//获取搜索数据
    this.refs.ckls.setState({
      Hes:0,
      page:1,
      hhs:[]
    });
    this.refs.ckls.handcbc();
  }
  handrest(){
    this.setState({
      sk:'',
          ...
          <ul>
            <li>收款人
              <Select showSearch
                      ...
                      onChange={(v)=>{return this.setState({sk:v})}}
              >
                {
                  this.props.pays.bhr.map(s => {
            ...
            <li>付款人
              <Input
                ...
                style={{ width: 120 }}
                placeholder="付款人账号"
                onChange={(e)=>{return this.setState({fk:e.target.value})}}
              />
            </li>
            <li>服务类型
              <Select showSearch
              ...
              >
                {
                  this.props.pays.bhfw.map(s => <Option key={s.serv}>{s.servName}</Option>)
                }
              </Select>
            </li>
            <li>付费期限
