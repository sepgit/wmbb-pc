...
import Adlistall from './adlistall';
...
export default class Adlists extends Component {
  ...
  handczqx(){
    let re=[];
    this.state.page==1?re=this.props.ysrdu.yslists:re=this.state.hhs;
    //设置子组件状态
    let THIS=this;
    re.map((item,index) => {
  ...
  componentDidMount(){
    this.props.actions.getyslistc(this.state.userName,this.state.token);//初始化列表
  }
  renderList(){
    let re=[];
    this.state.page==1?re=this.props.ysrdu.yslists:re=this.state.hhs;
    if(re.length==0){
      if(this.props.yts){
        ...
        return <li className="adl2one">您的普通运价优势列表暂无记录，请添加优势，以便让您的客户和潜在客户找到您的询价。</li>;
      }
    }else {
  ...
  handleScroll(v){
    ...
    if((bodyheight-this.state.Hes)<=(bodyheight/2)){
      this.state.page++;
      this.props.actions.getyslistgd( this.state.userName, this.state.token, this.state.page, ...this.props.Fstate);//获取搜索条件
      this.setState({
        hhs:[
          ...this.state.hhs,
          ...this.props.ysrdu.yslists
        ]
      });
      this.state.Hes=0;
    }
          ...
          <ul className="adlists1">
              <li className="adl1">
                  <ul>
                      ...
                      <li>服务类型</li>
                      <li className="adl9">承运商</li>
                      <li className="adl8">起运地</li>
                      <li className="adl8">目的地</li>
                      <li>发布人</li>
                      <li>接收人</li>
                      <li>状态</li>
                      <li>详情</li>
