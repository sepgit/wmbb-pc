...
export default class Xptsfwli extends Component {
    constructor(props) {
        super(props);
    ...
    handxq(){
        this.props.actions.getfwdel(this.state.userName,this.state.token,this.props.rows.cont);
        this.props.handxq();
    }
    handxz(){
        if(this.state.prov==1) {
            this.props.actions.getusedelfw(this.state.userName, this.state.token, this.props.rows.user);//获取联系人
            //添加供应商
            this.props.handgys();
        }else{
    ...
    render() {
        ...
        return (
            <li>
                ...
                <div className="xpts6" title={this.props.rows.name}>
                    {this.props.rows.name}
                </div>
                <div className="xpts7" title={this.props.rows.mobi}>
