...
export function getssgc(userName,token,pageIndex,serv,depaPort,destPort,cabSt,carr){
  return function(dispatch) {
    fetch(HTTPED+'api/cabRepls/?userName='+userName+'&token='+token+"&rowCount=10&pageIndex="+pageIndex+"&serv="+serv+"&depaPort="+depaPort+"&destPort="+destPort+"&cabSt="+cabSt+"&carr="+carr,{
      method: "get",
      headers: {
        "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
      }
...
function get_gcbz(date) {
  return {
    ...
    err:date.err,
    errMsg:date.errMsg,
    cabReplgcbz:date.cabRepl
  }
}

export function getgcbz(userName,token,cabRepl,replCLabe){
  return function(dispatch) {
    fetch(HTTPED+'api/cabRepls/'+cabRepl+'/',{
      method: "put",
      headers: {
      ...
      body: "userName="+userName+"&token="+token+"&isLabe=true&replCLabe="+replCLabe
    }).then(function(res){
      if(res.ok){
        res.json().then(function(date) {
          if(!date.err){
