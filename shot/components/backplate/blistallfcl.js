...
import Bdelfcl from './bdelfcl';
import Bchfcl from './bchfcl';
import Bzffcl from './bzffcl';
...
export default class Blistallfcl extends Component {
  constructor(props) {
    super(props);
  ...
  handxq(){
    ...
    //获取求舱保函详情
    let enqu=this.props.rows.enqu;
    this.props.actions.getcabrxq(this.state.userName,this.state.token,enqu);
    //获取供舱保函详情
    let repl=this.props.rows.repl;
    this.props.actions.getcwhps(this.state.userName,this.state.token,repl);
  }
  handchshow(k){
  ...
  render() {
    ...
    return (
      <li className="bplt">
        ...
        <div className="bplt2">
          <div className="bplta">
            ...
            <ul>
              ...
              <li className="bpltd">{this.props.rows.freiCurr}
                {
          ...
          <div className="bplte">
            ...
            <ul>{
              this.props.rows.GP20!=null&&this.props.rows.GP20!=0?
                <li>
                  <span>{this.props.rows.GP20}</span>
                  <img src={require('../../src/image/20gp.png')}/>
                </li>:undefined
            }
              {
                this.props.rows.GP40 != null&&this.props.rows.GP40 != 0 ?
                  <li>
                    <span>{this.props.rows.GP40}</span>
                    <img src={require('../../src/image/40gp.png')}/>
                  </li>:undefined
              }
              {
                this.props.rows.NOR40 != null&&this.props.rows.NOR40 != 0  ?
                  <li>
                    <span>{this.props.rows.NOR40}</span>
                    <img src={require('../../src/image/40nor.png')}/>
                  </li>:undefined
              }
              {
                this.props.rows.HQ40 != null&&this.props.rows.HQ40 != 0 ?
                  <li>
                    <span>{this.props.rows.HQ40}</span>
                    <img src={require('../../src/image/40hq.png')}/>
                  </li>:undefined
              }
              {
                this.props.rows.HQ45 != null&&this.props.rows.HQ45 != 0 ?
                  <li>
                    <span>{this.props.rows.HQ45}</span>
                    <img src={require('../../src/image/45hq.png')}/>
                  </li>:undefined
              }
            </ul>
          </div>
        ...
        <div className="bplt4">
          <a href="javascript:void(0);" onClick={this.handxq}>》</a>
        </div>
        <div className="xpg">
          {
        ...
        <VelocityTransitionGroup enter={{animation: "fadeIn",duration:1000}} leave={{animation: "fadeOut"}}>
          {
            this.props.shows.texthp&&this.props.shows.keyshp==this.props.keys?
              <Bdelfcl keysd={this.props.keys}
                       handchshow={this.handchshow}
        ...
        <VelocityTransitionGroup enter={{animation: "fadeIn",duration:1000}} leave={{animation: "fadeOut"}}>
          {
            this.state.isch&&this.state.keysd==this.props.keys?
              <Bchfcl handchclose={this.handchclose}
                      actions={this.props.actions}
                      bck={this.props.bck}
        ...
        <VelocityTransitionGroup enter={{animation: "fadeIn",duration:1000}} leave={{animation: "fadeOut"}}>
          {
            this.state.iszf&&this.state.keysd==this.props.keys?
              <Bzffcl handzfclose={this.handzfclose}
                      text={this.props.text}
