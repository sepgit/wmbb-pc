...
export default class Csr extends Component {
    ...
    handcheng(v){
        let encount=v.length;
        if(encount>5){
            message.error('抄送人不能超过5个人！')
        }else{
            this.setState({
                ccto:v
            });
            ...
            <div className="addsjr">
                ...
                <div className="addsjr4">
                    ...
                    <Select
                        ...
                        >
                        {
                            this.props.getnewlist.cctos.map((item, index)=> {
                                let strcctos = '{"user":"' + item.user + '","userAcco":"' + item.userAcco + '","name":"' + item.name + '","admi":"'+ item.admi+'"}';
