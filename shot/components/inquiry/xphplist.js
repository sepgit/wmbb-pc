...
export default class Xphplists extends Component {
  ...
  handcl(){
    ...
    //回盘详情的舱位部分
    this.props.actions.getxpxqch(this.state.userName,this.state.token,this.props.rows.repl);
    //回盘超期提醒
  ...
  handxqb(){
    this.setState({
      isgcx: !this.state.isgcx,
      ishplistshow:false
    });
  }
  render() {
    let GP20,GP40,NOR40,HQ40,HQ45,strGP20,strGP40,strNOR40,strHQ40,strHQ45;
    if(this.props.rows.GP20!=null&&this.props.rows.GP20!=0){
      GP20=this.props.rows.GP20;
      strGP20='20GP';
    }
    if(this.props.rows.GP40!=null&&this.props.rows.GP40!=0){
      GP40=this.props.rows.GP40;
      strGP40='40GP';
    }
    if(this.props.rows.NOR40!=null&&this.props.rows.NOR40!=0){
      NOR40=this.props.rows.NOR40;
      strNOR40='40NOR';
    }
    if(this.props.rows.HQ40!=null&&this.props.rows.HQ40!=0){
      HQ40=this.props.rows.HQ40;
      strHQ40='40HQ';
    }
    if(this.props.rows.HQ45!=null&&this.props.rows.HQ45!=0){
      HQ45=this.props.rows.HQ45;
      strHQ45='45HQ';
    }
    let styl='';
    ...
    const texts=(
      <div>
        {logos}
        <p className="infologop">信用金额启用：{this.props.getdetil.peohpinfo.depositEnab==1?'是':'否'}</p>
      </div>
    );
    const content = (
      <div>
    ...
    return (
      <li onClick={this.handcl} style={{border:styl}}>
        ...
        <div className="xppxa">
          <div className="xppxg">
            ...
            <ul>
              <li>{GP20}/{strGP20}</li>
              <li>{GP40}/{strGP40}</li>
              <li>{NOR40}/{strNOR40}</li>
              <li>{HQ40}/{strHQ40}</li>
              <li>{HQ45}/{strHQ45}</li>
            </ul>
            <div className="xppxf">
              {
                this.props.rows.cabEnqu > 0 ? <span onClick={this.handxqb}>舱位详情</span>:undefined
              }
            </div>
          </div>
        </div>
        ...
        <VelocityTransitionGroup enter={{animation: "fadeIn"}} leave={{animation: "fadeOut"}}>
          {
            this.state.isgcx?<Xphplistbzgc getdetil={this.props.getdetil}/>:undefined
          }
        </VelocityTransitionGroup>
      </li>
    );
