...
export default class Addgys extends Component {
  ...
  componentDidMount(){
    this.props.actions.getsera(this.state.userName,this.state.token);//所有服务
  }
  handch(){
    let userName=this.state.userName;
    let token=this.state.token;
    ...
    this.props.actions.postxjgys(userName,token,serv,port,cont,labe);//新增供应商
    this.props.handgysc();
  }
  handzf(){
    this.setState({
  ...
  changefw(v){
    ...
    this.props.actions.getfwkan(this.state.userName,this.state.token,v);//口岸
  }
  render() {
    return (
            ...
            <div className="supadd4">
              <ul>
                <li>
                  ...
                  <p>
                    {this.props.getnewlist.userdel.userAcco}
                  </p>
                </li>
                <li>
                  <span className="suph">姓名</span>
                  <p>
                    {this.props.getnewlist.userdel.name}
                  </p>
                </li>
                <li>
                  <span className="suph">公司</span>
                  <p>
                    {this.props.getnewlist.userdel.compAlia}
                  </p>
                </li>
                <li>
                  <span className="suph">行业</span>
                  <p>
                    {this.props.getnewlist.userdel.induName}
                  </p>
                </li>
                <li>
                  <span className="suph">手机</span>
                  <p>
                    {this.props.getnewlist.userdel.mobi}
                  </p>
                </li>
              </ul>
            </div>
            ...
            <div className="supadd5">
              <div className="supadd24">
                <div className="supadd6">
                  ...
                  >
                    {
                      this.props.getnewlist.xsera.map(s => <Option key={s.serv}>{s.servName}</Option>)
                    }
                  </Select>
                </div>
                <div className="supadd7">
                  ...
                  >
                    {
                      this.props.getnewlist.fwkan.map(s => <Option key={s.port} date={s.port} value={s.portName+'/'+s.chsName}>{s.portName+'/'+s.chsName}</Option>)
                    }
                  </Select>
                </div>
              </div>
