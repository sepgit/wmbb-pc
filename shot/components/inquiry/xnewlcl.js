...
export default class NewLCL extends Component {
  constructor(props) {
    super(props);
    ...
    if(depaPort==''||destPort==''){
      ...
      if(isbh){
        if(depo==undefined||enquTar==''||replTar==''||cabServ==undefined){
          ...
          this.props.actions.getxaddlcl(userid,userName,token,serv,depaPort,destPort,sendTo,ccto,mngr,match,compDate,carrs,tranship,packType,recr,wate,bulk,itemName,expiDate,enquMemo,isbh,depo,cabFee,enquTar,replTar,provType,cabServ,cabProv,cabEnqu,curr,teamAnge);
