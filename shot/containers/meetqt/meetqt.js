...
import Mqmid from './../../components/meetqt/mqmid';
import Gotop from './../../components/Galy/Gotop';
import {bindActionCreators} from 'redux';
import {connect} from 'react-redux';
import {getmeetWithdel} from './../../selectors/getmeet';
import {
  getyg,
  getqx,
  gethgerxx,
...
class Meetqt extends Component {
  constructor(props) {
    super(props);
    this.state= {
  ...
  componentDidMount(){
    let meetid=this.props.location.query.a;
    this.props.actions.getmeetqt(this.state.userName,this.state.token,meetid);//洽谈列表
  }
  render() {
    const { actions,text,getdetil,getnewlist,zxinfo,rsup,pays,ysrdu,yssfw,yssp,meetq,getmeet,cabnew,cabmynew} = this.props;
    let metary=[];
    let ids=this.props.location.query.a;
    getmeet.map(i => {
        if(i.meet==ids){
          metary=i;
        }
    ...
    return (
      <div className="meetqpage">
          ...
          <NewL actions={actions}
                ...
                cabmynew={cabmynew}
          />
          <Mqmid actions={actions} text={text} meetq={meetq} metary={metary} />
      </div>
    );
  }
}
...
Meetqt=connect(mapStateToProps,mapDispatchToProps)(Meetqt);
module.exports = Meetqt;