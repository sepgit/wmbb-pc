...
import Adslistall from './adslistall';
import { Checkbox } from 'antd';
import {VelocityComponent, VelocityTransitionGroup} from 'velocity-react';

export default class Adslists extends Component {
    constructor(props) {
        super(props);
    ...
    handczqx(){
        let re=[];
        this.state.page==1?re=this.props.yssfw.ysslists:re=this.state.hhs;
        //设置子组件状态
        let THIS=this;
        re.map((item,index) => {
    ...
    componentDidMount(){
        this.props.actions.getysslistc(this.state.userName,this.state.token);//初始化列表
    }
    renderList(){
        let re=[];
        this.state.page==1?re=this.props.yssfw.ysslists:re=this.state.hhs;
        if(re.length==0){
            if(this.props.yts){
                ...
                return <li className="adl2one">您的服务优势列表暂无记录，请添加优势，以便让您的客户和潜在客户找到您的询价。</li>;
            }
        }else {
    ...
    handleScroll(v){
        ...
        if((bodyheight-this.state.Hes)<=(bodyheight/2)){
            this.state.page++;
            this.props.actions.getysslistgd( this.state.userName, this.state.token, this.state.page, ...this.props.Fstate);//获取搜索条件
            this.setState({
                hhs:[
                    ...this.state.hhs,
                    ...this.props.yssfw.ysslists
                ]
            });
            this.state.Hes=0;
        }
